...
const manifestSchema = "nomen.source-provenance-manifest.v1"
const bomSchema = "nomen.source-bom.v1"
...
func main() {
	manifestPath := flag.String("manifest", "provenance/source-manifest.json", "manifest path relative to repository root")
	outputPath := flag.String("output", "provenance/source-bom.json", "generated BOM path relative to repository root")
	check := flag.Bool("check", false, "verify that the checked-in BOM is current")
	flag.Parse()
...
func run(manifestPath, outputPath string, check bool) error {
	...
	generated, err := buildBOM(classifier{manifest: m, sources: sources, imported: imported}, tracked, manifestPath)
	if err != nil {
		return err
	}
	...
	if check {
		...
		if !bytes.Equal(normalizeLineEndings(actual), wanted) {
			return errors.New("source-bom.json is stale; run `go run ./dev/source-inventory`")
		}
...
func validateManifest(m manifest) error {
	...
	for _, item := range m.Sources {
		if item.ID == "" || item.Repository == "" || item.Revision == "" || item.LicenseExpression == "" || item.LicenseReference == "" {
			return fmt.Errorf("source %q is incomplete", item.ID)
		}
		if item.ID == "nomen-native" && item.LicenseExpression != "LicenseRef-AngryVibes-LLC AND LicenseRef-shippin.ai" {
			return fmt.Errorf("nomen-native must be licensed only by AngryVibes LLC and shippin.ai")
		}
		if item.ID == "nomen-native" && (strings.Contains(item.LicenseExpression, "AGPL") || strings.Contains(item.LicenseExpression, "Apache-2.0")) {
			return fmt.Errorf("nomen-native must not absorb a third-party product license")
		}
	...
	rules := make(map[string]struct{}, len(m.PathRules))
...
func buildBOM(c classifier, tracked []string, manifestPath string) (bom, error) {
	type groupKey struct{ ruleID, sourceID, relationship, inspiredBy string }
	...
	for _, name := range tracked {
		...
		key := groupKey{result.RuleID, result.SourceID, result.Relationship, strings.Join(inspired, ",")}
		groups[key] = append(groups[key], name)
		summary[result.SourceID]++
		fmt.Fprintf(hash, "%s\x00%s\x00%s\x00%s\x00%s\x00%s\n", name, result.SourceID, result.Relationship, result.RuleID, c.sources[result.SourceID].LicenseExpression, key.inspiredBy)
	}
	...
	for _, key := range keys {
		...
		var inspired []string
		if key.inspiredBy != "" {
			inspired = strings.Split(key.inspiredBy, ",")
		}
		bomGroups = append(bomGroups, bomGroup{key.ruleID, key.sourceID, key.relationship, c.sources[key.sourceID].LicenseExpression, inspired, groups[key]})
	}
	...
	for _, id := range sourceIDs {
		summaries = append(summaries, sourceSummary{id, summary[id]})
	}
	return bom{bomSchema, filepath.ToSlash(manifestPath), c.manifest.BaselineImport.Commit, len(tracked), hex.EncodeToString(hash.Sum(nil)), append([]source(nil), c.manifest.Sources...), summaries, bomGroups}, nil
}
	...
	for _, rule := range c.manifest.PathRules {
		for _, pattern := range rule.Patterns {
			if matches(pattern, name) {
				return classification{rule.SourceID, rule.Relationship, rule.ID, append([]string(nil), rule.InspiredBy...)}, nil
			}
		}
	}
	if _, ok := c.imported[name]; ok {
		for _, override := range c.manifest.BaselineImport.Overrides {
			if matches(override.Pattern, name) {
				return classification{override.SourceID, override.Relationship, "baseline-import-override", nil}, nil
			}
		}
		return classification{c.manifest.BaselineImport.SourceID, c.manifest.BaselineImport.Relationship, "baseline-import", nil}, nil
	}
