...
func TestPasswordCheckCommand_GetPasswordCheckAndError(t *testing.T) {
	...
	tt := []struct {
		...
		expectedCheckType domain.VerificationType
		expectedError     error
	}{
		{
			...
			inputErr:          nil,
			fetchedUser:       domain.User{},
			expectedCheckType: &domain.VerificationTypeSucceeded{},
			expectedError:     nil,
		},
		{
			...
			expectedCheckType: &domain.VerificationTypeFailed{},
	...
	for _, tc := range tt {
		t.Run(tc.testName, func(t *testing.T) {
			...
			if tc.expectedError != nil {
				checkFailed := checkType.(*domain.VerificationTypeFailed)
				assert.NotZero(t, checkFailed.FailedAt)
			} else {
				checkSuccess := checkType.(*domain.VerificationTypeSucceeded)
				assert.NotZero(t, checkSuccess.VerifiedAt)
			}
		})
	}
}
	...
	tt := []struct {
		...
		},
		{
			...
			sessionRepo: func(ctrl *gomock.Controller) domain.SessionRepository {
				repo := domainmock.NewSessionRepo(ctrl)
				idCondition := repo.IDCondition("session-1")
				sessionFactorChange := repo.SetFactor(&domain.SessionFactorPassword{LastVerifiedAt: time.Now()})
				repo.EXPECT().
		...
		},
		{
			...
			sessionRepo: func(ctrl *gomock.Controller) domain.SessionRepository {
				repo := domainmock.NewSessionRepo(ctrl)
				idCondition := repo.IDCondition("session-1")
				sessionFactorChange := repo.SetFactor(&domain.SessionFactorPassword{LastFailedAt: time.Now()})
				repo.EXPECT().
					Update(gomock.Any(), gomock.Any(), idCondition, sessionFactorChange).
					Times(1).
					Return(int64(1), nil)
