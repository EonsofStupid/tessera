...
func TestServer_GetProject(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				ctx: iamOwnerCtx,
				req: &project.GetProjectRequest{ProjectId: "notexisting"},
			},
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, err := instance.Client.ProjectV2.GetProject(tt.args.ctx, tt.args.req)
				if tt.wantErr {
...
func TestServer_ListProjects(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					...
					request.Filters[0].Filter = &project.ProjectSearchFilter_InProjectIdsFilter{
						InProjectIdsFilter: &filter.InIDsFilter{
							Ids: []string{response.Projects[0].GetProjectId(), response.Projects[1].GetProjectId(), response.Projects[2].GetProjectId()},
						},
					}
				},
				req: &project.ListProjectsRequest{
		...
		},
		{
			...
			args: args{
				...
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					...
					request.Filters[0].Filter = &project.ProjectSearchFilter_InProjectIdsFilter{
						InProjectIdsFilter: &filter.InIDsFilter{
							Ids: []string{resp1.GetProjectId(), resp2.GetProjectId(), resp3.GetProjectId()},
						},
					}
		...
		},
		{
			...
			args: args{
				...
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					...
					request.Filters[0].Filter = &project.ProjectSearchFilter_InProjectIdsFilter{
						InProjectIdsFilter: &filter.InIDsFilter{
							Ids: []string{resp1.GetProjectId(), resp2.GetProjectId(), resp3.GetProjectId(), projectResp.GetProjectId()}},
					}
			...
			args: args{
				...
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					...
					request.Filters[0].Filter = &project.ProjectSearchFilter_OrganizationIdFilter{
						OrganizationIdFilter: &project.ProjectOrganizationIDFilter{OrganizationId: *grantedProjectResp.GrantedOrganizationId},
					}
				},
				req: &project.ListProjectsRequest{
					Filters: []*project.ProjectSearchFilter{{}},
		...
		{
			name: "list project and granted projects, own projects",
			args: args{
				ctx: iamOwnerCtx,
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					orgID := instance.DefaultOrg.GetId()
		...
		{
			name: "list project and granted projects, granted projects",
			args: args{
				ctx: iamOwnerCtx,
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instance.Client.ProjectV2.ListProjects(tt.args.ctx, tt.args.req)
				if tt.wantErr {
...
func TestServer_ListProjects_PermissionV2(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "list project and granted projects, owned and granted projects",
			args: args{
				ctx: iamOwnerCtx,
				dep: func(request *project.ListProjectsRequest, response *project.ListProjectsResponse) {
					projectResp := createProject(iamOwnerCtx, instancePermissionV2, t, orgID, true, true)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retryDuration, tick := integration.WaitForAndTickWithMaxDuration(iamOwnerCtx, time.Minute)
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instancePermissionV2.Client.ProjectV2.ListProjects(tt.args.ctx, tt.args.req)
				if tt.wantErr {
					require.Error(ttt, listErr)
					return
				}
...
func createGrantedProject(ctx context.Context, instance *integration.Instance, t *testing.T, projectToGrant *project.Project) *project.Project {
	...
	projectGrantResp := instance.CreateProjectGrant(ctx, t, projectToGrant.GetProjectId(), grantedOrg.GetOrganizationId())

	return &project.Project{
		ProjectId:               projectToGrant.GetProjectId(),
...
func TestServer_ListProjectGrants(t *testing.T) {
	...
	patResp := instance.CreatePersonalAccessToken(iamOwnerCtx, userResp.GetUserId())
	projectResp := createProject(iamOwnerCtx, instance, t, instance.DefaultOrg.GetId(), false, false)
	projectGrantResp := createProjectGrant(iamOwnerCtx, instance, t, instance.DefaultOrg.GetId(), projectResp.GetProjectId(), projectResp.GetName())
	instance.CreateProjectGrantMembership(t, iamOwnerCtx, projectResp.GetProjectId(), projectGrantResp.GetGrantedOrganizationId(), userResp.GetUserId())
	...
	tests := []struct {
		...
		args: args{
			...
			dep: func(request *project.ListProjectGrantsRequest, response *project.ListProjectGrantsResponse) {
				...
				request.Filters[1].Filter = &project.ProjectGrantSearchFilter_OrganizationIdFilter{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retryDuration, tick := integration.WaitForAndTickWithMaxDuration(iamOwnerCtx, time.Minute)
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instance.Client.ProjectV2.ListProjectGrants(tt.args.ctx, tt.args.req)
				if tt.wantErr {
...
func TestServer_ListProjectGrants_PermissionV2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retryDuration, tick := integration.WaitForAndTickWithMaxDuration(iamOwnerCtx, time.Minute)
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instancePermissionV2.Client.ProjectV2.ListProjectGrants(tt.args.ctx, tt.args.req)
				if tt.wantErr {
					require.Error(ttt, listErr, "Error: "+listErr.Error())
					return
				}
...
func createProjectGrant(ctx context.Context, instance *integration.Instance, t *testing.T, orgID, projectID, projectName string, roles ...string) *project.ProjectGrant {
	...
	require.EventuallyWithT(t, func(collect *assert.CollectT) {
		projects, err := instance.Client.ProjectV2.ListProjects(ctx, &project.ListProjectsRequest{
			Filters: []*project.ProjectSearchFilter{{
				Filter: &project.ProjectSearchFilter_InProjectIdsFilter{
				...
				Filter: &project.ProjectSearchFilter_OrganizationIdFilter{
					OrganizationIdFilter: &project.ProjectOrganizationIDFilter{OrganizationId: grantedOrg.GetOrganizationId()},
				},
			}},
		})
...
func TestServer_ListProjectRoles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retryDuration, tick := integration.WaitForAndTickWithMaxDuration(iamOwnerCtx, time.Minute)
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instance.Client.ProjectV2.ListProjectRoles(tt.args.ctx, tt.args.req)
				if tt.wantErr {
					require.Error(ttt, listErr, "Error: "+listErr.Error())
					return
				}
...
func TestServer_ListProjectRoles_PermissionV2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retryDuration, tick := integration.WaitForAndTickWithMaxDuration(iamOwnerCtx, time.Minute)
			require.EventuallyWithT(t, func(ttt *assert.CollectT) {
				got, listErr := instancePermissionV2.Client.ProjectV2.ListProjectRoles(tt.args.ctx, tt.args.req)
				if tt.wantErr {
					require.Error(ttt, listErr, "Error: "+listErr.Error())
					return
				}
