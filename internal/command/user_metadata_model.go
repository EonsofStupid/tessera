...
import (
	...
	"github.com/EonsofStupid/tessera/internal/repository/metadata"
...
func (wm *UserMetadataWriteModel) AppendEvents(events ...eventstore.Event) {
	for _, event := range events {
		...
		default:
			wm.MetadataWriteModel.AppendEvents(e)
		}
	}
}
...
func (wm *UserMetadataListWriteModel) AppendEvents(events ...eventstore.Event) {
	for _, event := range events {
		...
		default:
			wm.MetadataListWriteModel.AppendEvents(e)
		}
	}
}
...
func (wm *UserMetadataByOrgListWriteModel) Reduce() error {
	for _, event := range wm.Events {
		...
		case *metadata.RemovedAllEvent, *user.HumanAddedEvent, *user.HumanRegisteredEvent, *user.MachineAddedEvent, *user.UserRemovedEvent:
