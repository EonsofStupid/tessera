...
import (
	...
	protoc_gen_nomen "github.com/shippinAI/nomen/pkg/grpc/protoc/v2"
)

var (
	//go:embed nomen.pb.go.tmpl
	nomenTemplate []byte
)
...
func main() {
	...
	tmpl := loadTemplate(nomenTemplate)
	...
	for _, file := range plugin.Files {
		...
		for _, service := range file.Services {
			...
			for _, method := range service.Methods {
				...
				ext := proto.GetExtension(options, protoc_gen_nomen.E_Options).(*protoc_gen_nomen.Options)
				if ext == nil {
					continue
				}
...
func generateAuthOption(methods *authMethods, protoAuthOption *protoc_gen_nomen.AuthOption, method *protogen.Method) {
...
func generateFile(tmpl *template.Template, methods *authMethods, protoFile *protogen.File, plugin *protogen.Plugin) {
	...
	filename := protoFile.GeneratedFilenamePrefix + ".pb.nomen.go"
