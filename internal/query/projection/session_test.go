...
func TestSessionProjection_reduces(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			reduce: (&sessionProjection{}).reduceOwnerRemoved,
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "DELETE FROM projections.sessions8 WHERE (user_resource_owner = $1) AND (instance_id = $2)",
							expectedArgs: []any{
								"agg-id",
								"instance-id",
							},
