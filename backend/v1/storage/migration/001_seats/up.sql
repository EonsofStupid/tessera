...
--
-- Until now these lived in Zitadel user metadata, which is a key-value bag with
...
CREATE TYPE tessera.seat_occupant AS ENUM (
    'human'
    , 'agent'
);
...
CREATE TYPE tessera.seat_basis AS ENUM (
    'subscription'
    , 'usage'
    , 'local'
    , 'unknown'
...
CREATE TABLE tessera.seats (
    ...
    , occupant tessera.seat_occupant NOT NULL DEFAULT 'agent'
    , basis tessera.seat_basis NOT NULL DEFAULT 'unknown'
    ...
    , CONSTRAINT fk_seat_member FOREIGN KEY (instance_id, member_id)
        REFERENCES zitadel.users (instance_id, id) ON DELETE CASCADE
);

-- Which seats belong to an account: the panel's list view, per customer.
CREATE INDEX idx_seats_account ON tessera.seats (instance_id, account_id);
...
CREATE TABLE tessera.seat_workspaces (
    instance_id TEXT NOT NULL
    , member_id TEXT NOT NULL
    ...
    , CONSTRAINT fk_seat_workspace_seat FOREIGN KEY (instance_id, member_id)
        REFERENCES tessera.seats (instance_id, member_id) ON DELETE CASCADE
);
...
CREATE INDEX idx_seat_workspaces_workspace
    ON tessera.seat_workspaces (instance_id, workspace_id);

-- Our own, rather than calling zitadel.set_updated_at across the schema
-- boundary. Five lines to keep this schema standing on its own.
CREATE OR REPLACE FUNCTION tessera.set_updated_at() RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
END;
...
CREATE OR REPLACE TRIGGER trg_set_updated_at_seats
    BEFORE UPDATE ON tessera.seats
    FOR EACH ROW
