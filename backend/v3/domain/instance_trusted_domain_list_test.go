...
func TestListInstanceTrustedDomainsQuery_Validate(t *testing.T) {
	t.Parallel()
	permissionErr := errors.New("permission error")

	tt := []struct {
		name              string
		request           *instance.ListTrustedDomainsRequest
	...
	}{
		{
			...
			request: &instance.ListTrustedDomainsRequest{InstanceId: " "},
			permissionChecker: func(ctrl *gomock.Controller) domain.PermissionChecker {
		...
		},
		{
			...
			request: &instance.ListTrustedDomainsRequest{InstanceId: "instance-1"},
			permissionChecker: func(ctrl *gomock.Controller) domain.PermissionChecker {
				checker := domainmock.NewMockPermissionChecker(ctrl)
				checker.EXPECT().
					CheckInstancePermission(gomock.Any(), domain.DomainReadPermission).
		...
		},
		{
			...
			expectedError: zerrors.ThrowPermissionDenied(permissionErr, "DOM-nx0QKI", "Errors.PermissionDenied"),
		},
		{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			query := domain.NewListInstanceTrustedDomainsQuery(tc.request)
...
func TestListInstanceTrustedDomainsQuery_Execute(t *testing.T) {
	t.Parallel()
	...
	tt := []struct {
		...
		},
		{
					...
					List(gomock.Any(), gomock.Any(),
						dbmock.QueryOptions(
							database.WithCondition(database.And(
								...
								repo.TypeCondition(domain.DomainTypeTrusted),
							)),
						),
			...
			expectedError: zerrors.ThrowInternal(listErr, "DOM-8eoWqY", "Errors.Instance.TrustedDomain.List"),
		},
		{
...
func TestListInstanceTrustedDomainsQuery_Sorting(t *testing.T) {
	t.Parallel()
	tt := []struct {
		name    string
		request *instance.ListTrustedDomainsRequest
		...
		},
		{
			...
			request: &instance.ListTrustedDomainsRequest{
				SortingColumn: instance.TrustedDomainFieldName_TRUSTED_DOMAIN_FIELD_NAME_UNSPECIFIED,
			},
			expectedSortingDirection: database.OrderDirectionAsc,
		},
	}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := domain.NewListInstanceTrustedDomainsQuery(tc.request)
